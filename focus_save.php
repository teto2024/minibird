...
define('REWARD_MAX_COINS', 200000);               // 報酬として支給されるコイン数の最大値
define('REWARD_MAX_CRYSTALS', 500);            // 報酬として支給されるクリスタル数の最大値
...
// 報酬の上限を設定（不正報酬を防ぐ）
// --------------------------
// 成功または失敗による最終的な報酬が上限を超えないようチェック
$final_coins = min($final_coins, REWARD_MAX_COINS);
$final_crystals = min($final_crystals, REWARD_MAX_CRYSTALS);

// チート対策: 必要な最小作業時間のために以下を確認
$required_seconds = max(60 * $mins, 60); // 最低設定時間1分と同等
$actual_seconds_worked = max(1, strtotime($ended_at) - strtotime($started_at));

if ($actual_seconds_worked < $required_seconds * 0.3) { // 必要最低作業時間の30%未満なら不正と判断
    $final_coins = 0; // 報酬をゼロに
    $final_crystals = 0;
}
    // --------------------------
    // タスク履歴保存（開始時バフ倍率も記録）
