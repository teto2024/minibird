...
2. **重複排除（最適化版）**: `LEFT JOIN` + `IS NULL` で重複を防止
   - 第2のクエリは `LEFT JOIN conquest_castles ... WHERE cc.owner_user_id IS NULL` を使用
   - これにより、城を持っているプレイヤーは第2のクエリから除外される
   - 結果として、各プレイヤーはUNIONの片方にのみ表示される（重複なし）
   - 元の `NOT IN` サブクエリより高速でNULL値の扱いも正確
...
5. **コードの可読性**: `$seasonId` 変数を使用してパラメータを明確化（UNIONの各部分で1つずつ必要）
