...
function serialize_post($row, $uid, $pdo){
    ...
    $quoted_post = null;
    if (!empty($row['quote_post_id'])) { // ← DBのカラム名に対応
        $q = $pdo->prepare("
...
if ($action === 'fetch' || $action === 'fetch_more') {
    ...
    if ($feed === 'recommended') {
        $sql = "SELECT p.*, u.handle, p.deleted_at, p.deleted_by_mod,
                ...
                ORDER BY ((SELECT COUNT(*) FROM replies rpX WHERE rpX.post_id=p.id)*3 + (SELECT COUNT(*) FROM reposts rX WHERE rX.post_id=p.id)*2 + (SELECT COUNT(*) FROM likes lX WHERE lX.post_id=p.id)) DESC, p.id DESC
                LIMIT ?";
        $st = $pdo->prepare($sql);
        $st->execute([$limit]);
        $rows = $st->fetchAll(PDO::FETCH_ASSOC);
